...
var _ = Describe("Deploy Klusterlet with Multiplehubs enabled", func() {
	...
	It("should have expected resources deployed", func() {
		boostrapKubeConfigSecretHub1 := "bootstraphub1"
		boostrapKubeConfigSecretHub2 := "bootstraphub2"
		clusterName := "testcluster-multiplehubs"
		...
		By("Create Klusterlet related agentnamespace")
		...
		_, err = kubeClient.CoreV1().Secrets(klusterletNamespace).Create(context.TODO(), &corev1.Secret{
			ObjectMeta: metav1.ObjectMeta{
				Name: boostrapKubeConfigSecretHub1,
			},
